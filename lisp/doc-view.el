...
The function CONVERTER is called with the SOURCE, TARGET, PAGE,
and nil resolution arguments as a base, and additionally called
with the target and resolution arguments given by
...
are finished."
  (if (null doc-view-additional-conversion-targets-function)
      (funcall converter source target page callback nil)
    (let* ((all-targets
	...
	(funcall converter source (car target-resolution) page callback-once
		 (cdr target-resolution))))))

(defun doc-view-pdf->png-converter-ghostscript-1 (pdf png page callback
						      &optional resolution)
  "Like original `doc-view-pdf->png-converter-ghostscript', but accepts RESOLUTION.
This function is added by Emacs Mac port for high-resolution support."
...
(defun doc-view-pdf->png-converter-ghostscript (pdf png page callback)
  (doc-view-multiplex-conversion
   'doc-view-pdf->png-converter-ghostscript-1 pdf png page callback))

(defalias 'doc-view-ps->png-converter-ghostscript
  'doc-view-pdf->png-converter-ghostscript)

(defun doc-view-djvu->tiff-converter-ddjvu-1 (djvu tiff page callback
						   &optional resolution)
  "Like original `doc-view-djvu->tiff-converter-ddjvu', but accepts RESOLUTION.
This function is added by Emacs Mac port for high-resolution support."
  (doc-view-start-process
   "djvu->tiff" "ddjvu"
   `("-format=tiff"
...
If PAGE is nil, convert the whole document."
  (doc-view-multiplex-conversion
   'doc-view-djvu->tiff-converter-ddjvu-1 djvu tiff page callback))

(defun doc-view-pdf->png-converter-mupdf-1 (pdf png page callback
						&optional resolution)
  "Like original `doc-view-pdf->png-converter-mupdf', but accepts RESOLUTION.
This function is added by Emacs Mac port for high-resolution support."
  (doc-view-start-process
...
(defun doc-view-pdf->png-converter-mupdf (pdf png page callback)
  (doc-view-multiplex-conversion
   'doc-view-pdf->png-converter-mupdf-1 pdf png page callback))
