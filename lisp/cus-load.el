...
(put 'tramp 'custom-loads '(tramp tramp-gvfs tramp-fish tramp-ftp tramp-imap tramp-cache tramp-smb))
...
(put 'languages 'custom-loads '(ps-mode ada-mode antlr-mode asm-mode cus-edit cfengine cperl-mode css-mode dcl-mode delphi f90 fortran hideshow icon idlwave info-look js ld-script m4-mode meta-mode modula2 nxml-mode octave-mod pascal perl-mode prolog python rng-valid sgml-mode sh-script sieve simula smie tcl vera-mode verilog-mode vhdl-mode))
...
(put 'wp 'custom-loads '(bib-mode delim-col ebnf2ps enriched lpr nroff-mode nxml-mode printing ps-print refbib refer rng-valid rst table cus-edit tildify view))
...
(put 'comm 'custom-loads '(dig eudc-vars ldap net-utils netrc tls tramp))
(put 'convenience 'custom-loads '(autoinsert autorevert bs calculator completion cua-base dabbrev ffap filecache filesets follow hippie-exp ibuffer ido imenu iswitchb kmacro linum master org-protocol complete pc-select repeat ruler-mode speedbar whitespace windmove))
...
(put 'semantic 'custom-loads '("mode" "util-modes" "idle" "mru-bookmark" "c" semantic "edit" "grammar" "imenu" "lex" "lex-spp" "grep" "list" "db" "senator" "wisent"))
...
(put 'srecode 'custom-loads '("document" "mode" "cpp" "expandproto" "fields" "insert" "map" "srt-mode"))
...
(put 'applications 'custom-loads '(calc calendar doc-view erc eshell cus-edit htmlfontify ispell mpc newst-backend rcirc ses spell uniquify))
...
(put 'data 'custom-loads '(arc-mode conf-mode dns-mode doc-view forms generic-x hexl remember saveplace snmp-mode sort tar-mode time-stamp timeclock))
